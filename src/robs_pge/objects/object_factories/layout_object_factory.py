...
class LayoutObjectFactory(SubObjectFactory):
    ...
    def grid_layout(
            self, position: vec2, width: Optional[int] = None, height: Optional[int] = None, style: StyleOrName[RectStyle] = None,
    ...
    def stack_horizontal(
        ...
        layout = self.grid_layout(position, width, height, style, rotation, scale, layer, anchor, cache)
    ...
    def stack_in_grid(
            ...
            mode: Literal["grid", "rows", "columns"] = LayoutObject.GRID_MODE, style: StyleOrName[RectStyle] = None,
            rotation: float = 0.0, scale: float = 1.0, layer: int = 0, anchor: vec2 = Anchor.C, cache: bool = True
    ):
